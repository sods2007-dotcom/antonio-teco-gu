...
def fim_partida_screen(window, partida, fase_torneio):
    ...
    Se venceu e nao eh final, oferece continuar. Se venceu a final, vai
    para CAMPEAO. Se perdeu, oferece voltar ao menu.
    """
    ...
    empate = partida.gols_jogador() == partida.gols_adversario()

    if empate:
        # So acontece em casos inesperados (morte subita evita empate)
        texto_principal = "EMPATE"
        cor_principal = WHITE
        proximo_state = MENU
        texto_botao = "VOLTAR AO MENU"
    elif venceu and e_final:
    ...
    while titulo.get_width() > WIDTH - 40:
        nova = pygame.font.SysFont(None, int(titulo.get_height() * 0.9))
        titulo = nova.render(texto_principal, True, cor_principal)
