"""
Tela de vitoria do torneio — confetes e trofeu.
"""
import pygame
import random
from config import (
    WIDTH, HEIGHT, FPS, WHITE, BLACK, GREEN_GRAMA, YELLOW, RED, GREEN,
    MENU, QUIT
)
...
def campeao_screen(window, time_jogador):
    """
    Mostra a tela de campeao com animacao de confetes e trofeu.
...
def criar_confete():
    ...
    return {
        ...
        'cor': (random.randint(100, 255), random.randint(100, 255),
...
def desenha_confete(window, c):
    ...
    s = pygame.Surface((c['tamanho'] * 2, c['tamanho']), pygame.SRCALPHA)
    pygame.draw.rect(s, c['cor'], (0, 0, c['tamanho'] * 2, c['tamanho']))
...
def desenha_trofeu(window, y_centro):
    ...
    cx = WIDTH // 2
    pygame.draw.rect(window, (100, 70, 30),
                    (cx - 40, y_centro + 60, 80, 20))
    pygame.draw.rect(window, YELLOW, (cx - 8, y_centro + 30, 16, 30))
    pygame.draw.ellipse(window, YELLOW,
                       (cx - 50, y_centro - 50, 100, 100))
    pygame.draw.ellipse(window, (255, 255, 200),
                       (cx - 35, y_centro - 40, 25, 50))
    pygame.draw.arc(window, YELLOW,
                   (cx - 70, y_centro - 30, 30, 50), 0.5, 2.5, 6)
    pygame.draw.arc(window, YELLOW,
                   (cx + 40, y_centro - 30, 30, 50), -1.5, 0.5, 6)